...
export default {
  ...
  mounted() {
    if (this.PopupId !== '0') {
      this.title = "Cập nhật thông tin Popup"
      this.formType = 'edit'
      this.loadFormEdit()
    } else {
      this.title = "Tạo mới Popup"
  ...
  methods: {
    ...
    async loadFormEdit() {
      try {
        this.loading = true
        const data = await PopupAPI.getById(this.PopupId)
        ...
        if (valid) {
          try {
            ...
            if (this.formType === 'create') {
              await PopupAPI.create(this.formSubmit)
            } else {
              await PopupAPI.update(this.formSubmit, this.PopupId)
            }
            this.loading = false
          } catch (err) {
            this.loading = false
