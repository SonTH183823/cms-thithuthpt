...
<template>
  <div class="app-container">
    <el-dialog
      :title="formType ==='create' ? 'Thêm xuất xứ mới' : 'Chỉnh sửa nơi xuất xứ'"
      :visible.sync="dialogFormVisible"
      :close-on-click-modal=false
      width="60%"
    >
      <el-form ref="form" :model="form" :rules="formRules" label-width="60px" label-position="top">
        <el-form-item class="banner-attri" label="Nơi xuất xứ" prop="source">
      ...
      <template slot="table" slot-scope="scope">
        ...
        >
          <el-table-column
            ...
            :index="indexMethod"
          />
          <el-table-column label="Nơi xuất xứ" prop="source" align="center" min-width="120"></el-table-column>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formRules: {
        source: [{required: true, trigger: 'blur', validator: validateText}],
  ...
  methods: {
    ...
    async onChangeActive(data) {
      const d = await OriginAPI.getById(data.id)
      const dataSubmit = {
        ...d,
      }
      try {
        this.loading = true
        await OriginAPI.update(data.id, dataSubmit)
        this.loading = false
      } catch (err) {
        this.loading = false
      }
    ...
    async handleDelete(prams) {
      try {
        this.loading = true
        await OriginAPI.delete(prams._id)
        this.loading = false
        ...
        if (valid) {
          switch (this.formType) {
            ...
            case 'edit':
              ...
              await OriginAPI.update(this.form._id, dataSubmit)
