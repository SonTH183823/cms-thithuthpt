...
<template>
  <div class="app-container">
    <el-dialog
      :title="formType ==='create' ? 'Thêm hệ thống dẫn động mới' : 'Chỉnh sửa hệ thống dẫn động'"
      :visible.sync="dialogFormVisible"
      :close-on-click-modal=false
      width="60%"
    >
      <el-form ref="form" :model="form" :rules="formRules" label-width="60px" label-position="top">
        <el-form-item class="banner-attri" label="Tên hệ thống dẫn động" prop="name">
      ...
      <template slot="table" slot-scope="scope">
        ...
        >
          <el-table-column
            ...
            :index="indexMethod"
          />
          <el-table-column label="Tên hệ thống dẫn động" prop="name" align="center" min-width="120"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async onChangeActive(data) {
      const d = await DriveAPI.getById(data.id)
      const dataSubmit = {
        ...d,
      }
      try {
        this.loading = true
        await DriveAPI.update(data.id, dataSubmit)
        this.loading = false
      } catch (err) {
        this.loading = false
      }
    ...
    async handleDelete(prams) {
      try {
        this.loading = true
        await DriveAPI.delete(prams._id)
        this.loading = false
        ...
        if (valid) {
          switch (this.formType) {
            ...
            case 'edit':
              ...
              await DriveAPI.update(this.form._id, dataSubmit)
