<template>
  <div v-loading="loading">
    <div class="news-form-container">
      <el-row><h2 style="font-weight: bold">Biên tập tài liệu</h2></el-row>
      <el-row style="margin-top: 12px;margin-bottom: 20px">
        <el-col style="display: flex; justify-content: flex-end; margin-right: 10px">
          ...
          <el-button type="primary" class="btn-submit" @click="handleSend">
            {{ formType !== 'edit' ? 'Tạo tài liệu' : 'Cập nhật' }}
          </el-button>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        ...
        if (valid) {
          try {
            ...
            if (this.formType === 'create') {
              await DocumentAPI.create(dataSubmit)
            } else {
              await DocumentAPI.update(dataSubmit, this.DocumentID)
            }
            this.formSubmit = {}
            this.loading = false
          } catch (err) {
          ...
          this.$router.push('/quan-ly-tai-lieu/danh-sach')
        } else {
