...
export default {
  ...
  methods: {
    async getLeadStatistical(params) {
      this.loading = true
      const res = await SellCarCustomerAPI.getStatistic(params)
        ...
        if (this.listState.length > 0) {
          this.listState.map(item => {
            if (typeof item.from === 'string') {
              ...
              this.optionsBarState.xaxis.categories.push(config.stateCarSale[item.from - 1].label)
              this.optionsPieState.labels.push(config.stateCarSale[item.from - 1].label)
            }
