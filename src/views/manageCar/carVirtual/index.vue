  ...
  <div class="app-container">
    <div class="list-btn" style=" width: 100%; display: flex; flex-direction: row">
      <a-button
        class="btn-import"
        type="primary"
        style="height: 36px; background-color: #e6a23c; color: white"
        @click="exportExcel()"
      >
        Export Excel file
      </a-button>
        ...
        >
          ...
          <el-table-column label="Giá dự kiến / Giá thực tế" min-width="140">
            <template slot-scope="scope">
              <div>
                <li>
                  Giá dự kiến: {{ scope.row.price.toLocaleString('it-IT', { style: 'currency', currency: 'VND' }) }}
                </li>
                <li v-if="scope.row.listedPrice">
                  Giá thực tế:
          ...
          <el-table-column label="Thông số kỹ thuật" align="center" min-width="400">
            <template slot-scope="scope">
              <div>
                <el-row>
                  <el-col :span="12">
    ...
    >
      ...
      <h3 v-if="tableDataEror.length > 0"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push('/quan-ly-xe/chi-tiet-xe/0')
    },
    openImportDialog() {
    ...
    async exportExcel() {
      try {
        this.loading = true
        const response = await CarAPI.getExportExcel({ copied: 1 })
        const url = window.URL.createObjectURL(new Blob([response.data], { type: response.data.type }))
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', `ds_xe_ao_${moment().format("DD_MM_YYYY_HH_mm_ss")}.xlsx`)
        document.body.appendChild(link)
        link.click()
        this.loading = false
      } catch (err) {
    ...
    handleEdit(row) {
      this.$router.push('/quan-ly-xe/chi-tiet-xe/' + row._id)
    },
    handleCheckCar(row) {
      this.$router.push({ path: '/quan-ly-xe/kiem-dinh-xe/' + row.id, query: { type: 'virtual' } })
    },
